...
export default function DashboardPage() {
  ...
  const { currentWeek, history, spendableBalance, wallet, markPaid, refetch: refetchAllowance } = useAllowance(kidId);
      ...
      <div className="flex items-start justify-between mb-4 flex-shrink-0">
        ...
        <div className="flex items-center gap-3 bg-surface-container-low p-2 pr-6 rounded-full shadow-card">
          ...
          <div>
            ...
            <p className="font-headline text-lg font-black text-on-surface">${(wallet?.totalHoldings ?? spendableBalance).toFixed(2)}</p>
          </div>
        </div>
      </div>
